...
public class PathPlannerTrajectory extends Trajectory {
  ...
  public PathPlannerTrajectory(List<Waypoint> pathPoints, List<EventMarker> markers,
  ...
  public PathPlannerTrajectory(List<State> states, List<EventMarker> markers,
  ...
  private static List<State> generatePath(List<Waypoint> pathPoints, double maxVel, double maxAccel,
      boolean reversed) {
  ...
  private static void recalculateValues(List<PathPlannerState> states, boolean reversed) {
    for (int i = states.size() - 1; i >= 0; i--) {
      ...
      if (i != states.size() - 1) {
        ...
        now.angularVelocityRadPerSec = MathUtil.inputModulus(
            next.poseMeters.getRotation().getRadians() - now.poseMeters.getRotation().getRadians(),
            -Math.PI, Math.PI) / dt;
        now.holonomicAngularVelocityRadPerSec = MathUtil.inputModulus(
            next.holonomicRotation.getRadians() - now.holonomicRotation.getRadians(), -Math.PI,
            Math.PI) / dt;
      }
  ...
  private static List<PathPlannerState> joinSplines(List<Waypoint> pathPoints, double maxVel,
    ...
    for (int i = 0; i < numSplines; i++) {
      ...
      for (double t = 0; t <= endStep; t += step) {
        ...
        double holonomicRot = GeometryUtil
            .cosineInterpolate(startRot, Rotation2d.fromDegrees(startRot.getDegrees() + deltaRot),
        ...
        if (i > 0 || t > 0) {
          ...
          if (i == 0 && t == step) {
            states.get(states.size() - 1).poseMeters =
                new Pose2d(states.get(states.size() - 1).poseMeters.getTranslation(),
  ...
  private void calculateMarkerTimes(List<Waypoint> waypoints) {
    for (EventMarker marker : this.markers) {
      ...
      marker.positionMeters = GeometryUtil.cubicLerp(startPoint.anchorPoint, startPoint.nextControl,
          endPoint.prevControl, endPoint.anchorPoint, t);
  ...
  public static class PathPlannerState extends State {
    ...
    private PathPlannerState interpolate(PathPlannerState endVal, double t) {
      ...
      lerpedState.accelerationMetersPerSecondSq = GeometryUtil
          .doubleLerp(accelerationMetersPerSecondSq, endVal.accelerationMetersPerSecondSq, t);
  ...
  public static class Waypoint {
    ...
    public Waypoint(Translation2d anchorPoint, Translation2d prevControl, Translation2d nextControl,
        double velOverride, Rotation2d holonomicRotation, boolean isReversal, boolean isStopPoint,
        StopEvent stopEvent) {
