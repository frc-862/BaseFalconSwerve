...
public class PPHolonomicDriveController {
        ...
        public boolean atReference() {
                ...
                return Math.abs(this.translationError.getX()) < translationTolerance.getX()
                                && Math.abs(this.translationError.getY()) < translationTolerance
                                                .getY()
                                && Math.abs(this.rotationError.getRadians()) < rotationTolerance
                                                .getRadians();
        }
        ...
        public ChassisSpeeds calculate(Pose2d currentPose, PathPlannerState referenceState) {
                ...
                this.translationError =
                                referenceState.poseMeters.relativeTo(currentPose).getTranslation();
                this.rotationError =
                                referenceState.holonomicRotation.minus(currentPose.getRotation());
                ...
                double xFeedback = this.xController.calculate(currentPose.getX(),
                                referenceState.poseMeters.getX());
                double yFeedback = this.yController.calculate(currentPose.getY(),
                                referenceState.poseMeters.getY());
                double rotationFeedback = this.rotationController.calculate(
                                currentPose.getRotation().getRadians(),
                                referenceState.holonomicRotation.getRadians());
                double poseFeedBackX = this.poseController.calculate(currentPose.getX(),
                                referenceState.poseMeters.getX());
                double poseFeedBackY = this.poseController.calculate(currentPose.getY(),
                                referenceState.poseMeters.getY());

                return ChassisSpeeds.fromFieldRelativeSpeeds(xFF + xFeedback + poseFeedBackX,
                                yFF + yFeedback + poseFeedBackY, rotationFF + rotationFeedback,
                                currentPose.getRotation());
        }
}
